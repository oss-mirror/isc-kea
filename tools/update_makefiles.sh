...
extract_includes() {
	...
	for i in ${INCLUDES_LIST}; do
		if test $(echo "${LIBRARIES_LIST}" | grep "${i}" | wc -l) -ne 0; then
			FILTERED_INCLUDES_LIST="${FILTERED_INCLUDES_LIST} ${i}"
		fi
	done
...
extract_non_include_files() {
	...
	RAW_INCLUDES_LIST=$(echo "${RAW_INCLUDES_LIST}" | cut -d "#" -f 2 | tr "\"" " " | cut -d "<" -f 2 | cut -d ">" -f 1 | grep "\/" | grep "${2}" | cut -d "/" -f 2 | grep "\." | sort | uniq)
...
for i in ${LIBRARIES_LIST}; do
	...
	for j in ${NON_RECURSIVE_BASE_DEPENDENCIES}; do
		NEW_DEPENDENCIES=$(extract_dependencies "${REVERSE_LIBRARIES_LIST}" "${j}")
		if test $(echo ${NEW_DEPENDENCIES} | wc -w) -gt $(echo ${DEPENDENCIES} | wc -w); then
