...
#include <map>
#include <vector>
#include <relay/relay_parser.h>
#include <relay/parser_context_decl.h>
#include <exceptions/exceptions.h>

// Tell Flex the lexer's prototype ...
#define YY_DECL isc::relay::RelayParser::symbol_type relay_lex (ParserContext& driver)
...
/// parse only content of the Relay object, which is a subset
...
public:
    ...
    ///
    /// RelayParser is able to parse several types of scope. Usually,
    /// when it parses a config file, it expects the data to have a map
    /// with Relay in it and all the parameters within that map.
    ...
    typedef enum {
        ...
        /// This parser will expect the content as Relay config wrapped
        /// in a map (that's the regular config file)
        PARSER_RELAY,

        /// This parser will expect only the content of Relay.
        PARSER_SUB_RELAY
    } ParserType;
    ...
    ///
    /// @param str string to be parsed
    /// @param parser_type specifies expected content (usually RELAY or generic JSON)
    ...
    ///
    /// @param filename file to be parsed
    /// @param parser_type specifies expected content (usually PARSER_RELAY or
    ///                                                PARSER_JSON)
    /// @return Element structure representing parsed text.
    ...
    /// @throw ParseError
    void error(const isc::relay::location& loc,
    ...
    /// cases when the RelayParser is not able to handle the packet.
    ///
    /// @param what string explaining the nature of the error.
    /// @throw ParseError
    ...
    isc::data::Element::Position loc2pos(isc::relay::location& loc);
    ...
    typedef enum {
        ...
        ///< Used while parsing top level (that contains Relay)
        CONFIG,

        ///< Used while parsing content of Relay.
        RELAY,

        ///< Used while parsing Relay/hooks-libraries.
        HOOKS_LIBRARIES,

        ///< Used while parsing Relay/loggers structures.
        LOGGERS,

        ///< Used while parsing Relay/loggers/output_options structures.
        OUTPUT_OPTIONS
    } LexerContext;
    ...
    /// for logging errors.
    isc::relay::location loc_;

    /// @brief Location stack
    std::vector<isc::relay::location> locs_;
    ...
    /// does not parse properly. Relay parser uses simplified
