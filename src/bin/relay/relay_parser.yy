...
%require "3.0.0"
%defines
%define parser_class_name {RelayParser}
%define api.prefix {relay_}
%define api.token.constructor
%define api.value.type variant
%define api.namespace {isc::relay}
%define parse.assert
%code requires
{
#include <string>
...
using namespace isc::relay;
using namespace isc::data;
using namespace std;
}
// The parsing context.
%param { isc::relay::ParserContext& ctx }
%locations
%define parse.trace
...
// Actual regexps for tokens are defined in relay_lexer.ll.
%token
  END  0  "end of file"
  COMMA ","
  COLON ":"
  ...
  // (START_RELAY), part of the grammar related to relay (START_SUB_RELAY)
...
// consists of only Relay entry in one big { }.
%start start;
...
start: START_JSON      { ctx.ctx_ = ctx.NO_KEYWORDS; } json
     | START_RELAY     { ctx.ctx_ = ctx.CONFIG; } relay_syntax_map
     | START_SUB_RELAY { ctx.ctx_ = ctx.RELAY; } sub_relay
     ;
...
// the Relay. This is very useful for unit-testing, so we don't need
// to repeat the outer map and "Relay" map. We can simply provide
// the contents of that map.
sub_relay: LCURLY_BRACKET {
    // Parse the Relay map
...
// This defines the top-level { } that holds only Relay object.
relay_syntax_map: LCURLY_BRACKET {
    // This code is executed when we're about to start parsing
    // the content of the map
    ElementPtr m(new MapElement(ctx.loc2pos(@1)));
    ctx.stack_.push_back(m);
...
// This represents the single top level entry, i.e. Relay.
global_object: RELAY {
    ...
    // Ok, we're done with parsing Relay. Let's take the map
    // off the stack.
    ctx.stack_.pop_back();
    ctx.leave();
};
...
void
isc::relay::RelayParser::error(const location_type& loc,
