...
NetconfAgent::initSysrepo() {
    ...
    // Retrieve names and revisions of installed modules through from sysrepo.
    getModules();
}
...
void NetconfAgent::getModules() {
    ...
    for (S_Module const& module : modules) {
        ...
        modules_.try_emplace(name, revision);
    }
}
...
NetconfAgent::subscribeConfig(const CfgServersMapPair& service_pair) {
    ...
    auto cb = [&](sysrepo::S_Session sess,
                  const char *module_name,
                  const char *xpath,
                  sr_event_t event,
                  uint32_t /* request_id */) {
        NetconfAgentCallback callback(service_pair);
        return callback.module_change(sess, module_name, xpath, event, nullptr);
    };
    try {
...
NetconfAgent::subscribeToNotifications(const CfgServersMapPair& service_pair) {
    ...
    auto event_notif_callback =
        [&](sysrepo::S_Session session,
            ...
            NetconfAgentCallback callback(service_pair);
            return callback.event_notif(session, notification_type, path, vals,
                                        timestamp, nullptr);
        };
    auto event_notif_tree_callback =
        [&](sysrepo::S_Session session,
            ...
            NetconfAgentCallback callback(service_pair);
            return callback.event_notif_tree(session, notification_type,
                                             notification, timestamp,
                                             nullptr);
        };
    try {
        subs->event_notif_subscribe(service_pair.second->getModel().c_str(),
                                    event_notif_callback);
        subs->event_notif_subscribe_tree(
            service_pair.second->getModel().c_str(), event_notif_tree_callback);
    } catch (const std::exception& ex) {
        ostringstream msg;
        msg << "module change subscribe failed with " << ex.what();
        LOG_ERROR(netconf_logger, NETCONF_SUBSCRIBE_CONFIG_FAILED)
