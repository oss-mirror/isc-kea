...
/// @todo disabled: any "set_item" on keatest-module throws
/// sysrepo::sysrepo_exception "libyang[0]: Invalid arguments - list without
/// keys (lyd_find_sibling())."
TEST(YangReprTest, getTest) {
    ...
    // Cleanup.
    EXPECT_NO_THROW_LOG(sess->delete_item("/keatest-module:container"));
    EXPECT_NO_THROW_LOG(sess->apply_changes());
    EXPECT_NO_THROW_LOG(sess->delete_item("/keatest-module:main"));
    EXPECT_NO_THROW_LOG(sess->apply_changes());
